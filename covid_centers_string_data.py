...
# Equivalent data
# stored_covid_centers_data = [{'1name': ('Ruby Hall Clinic',),
...
# pprint(stored_covid_centers_data)
def generate_covid_centers_Data_to_HTML():
    # final string to be returned into html
    final_string = []

    start_string = "<div><ul class='list-group'><li class='list-group-item'>"
    end_string = "</li></ul></div>"

    for item in stored_covid_centers_data:
        ...
        place_local_num_html = f"<a href='tel:{place_local_num}'><button type='button' class='btn btn-info mx-2'>{place_local_num}</button></a>"
        each_div.append(place_local_num_html)

        # place_inter_num_html
        place_inter_num_html = f"<a href='tel:{place_inter_num}'><button type='button' class='btn btn-info mx-2'>{place_inter_num}</button></a>"
